...
const PizzaCart = ({
	...
	const renderSizesDough = (sizzesArr) => {
		return sizzesArr.map((size) => {
			const sizesClass = classNames(
				'cart__options-btn cart__options-btn_size',
				{
	...
	return (
		<div className='grid-pizza__cart cart' key={uuid()}>
			...
			<div className='cart__prise-add'>
				...
				<button className='cart__add-btn' onClick={handleAddPizza}>
					<span>+</span>Добавить
					{handleCountPizzaOnButton(items) > 0 ? (
						<span className='cart__count-pizza'>
							{handleCountPizzaOnButton(items)}
						</span>
					) : null}
				</button>
			</div>
